...
class RandomCorruption:
    def __call__(self, img):
        if random.random() < 0.5:
            return get_corruption(
                random.choice(CORRUPTION_TYPES),
...
if __name__ == "__main__":
    ...
    RESULTS_FILE = RESULTS_DIR / f"baseline_cnn_degaug_seed{args.seed}_metrics.csv"
    BEST_MODEL_FILE = MODELS_DIR / f"baseline_cnn_degaug_seed{args.seed}_best.pth"
    FINAL_MODEL_FILE = MODELS_DIR / f"baseline_cnn_degaug_seed{args.seed}_final.pth"
    CONFIG_FILE = RESULTS_DIR / f"baseline_cnn_degaug_seed{args.seed}_config.json"
